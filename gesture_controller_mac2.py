...
class GestureController:
    ...
    def _detect_gestures(self, landmarks, frame):
        ...
        current_palm_open = self._is_palm_open(landmarks_list)
        
        # If palm was open and now is closed, take screenshot
        if self.palm_open and not current_palm_open and self._is_palm_closed(landmarks_list):
        ...
        self.palm_open = current_palm_open
        
        # Then check other gestures
    ...
    def _is_palm_open(self, landmarks):
        ...
        finger_dips = [7, 11, 15, 19]  # Second joints
        
        # Check if all fingertips are above their second joints (extended)
        for tip, dip in zip(finger_tips, finger_dips):
            if landmarks[tip][1] > landmarks[dip][1]:  # If tip is below dip, finger is bent
                return False
        
        # Check thumb separately - it should be extended outward
        thumb_tip = landmarks[4]
        thumb_ip = landmarks[3]
        
        # Thumb is open if it's extended outward (x coordinate difference)
        thumb_open = (thumb_tip[0] - thumb_ip[0]) > 20  # Adjust threshold as needed
        
        return thumb_open
    
    def _is_palm_closed(self, landmarks):
        # Check if all fingers are closed (fist)
        finger_tips = [8, 12, 16, 20]  # Tips of fingers
        finger_mcps = [5, 9, 13, 17]   # Base of fingers
        
        # All fingertips should be close to their MCP joints
        for tip, mcp in zip(finger_tips, finger_mcps):
            if np.linalg.norm(np.array(landmarks[tip]) - np.array(landmarks[mcp])) > 50:
                return False
        
        # Thumb should be touching the side of the hand
        thumb_tip = landmarks[4]
        index_mcp = landmarks[5]
        return np.linalg.norm(np.array(thumb_tip) - np.array(index_mcp)) < 40
